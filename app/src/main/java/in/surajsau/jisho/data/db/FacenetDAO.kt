...
@Dao
interface FacenetDAO {

    @Query("SELECT * FROM faceimage WHERE isPrimary = 1")
    suspend fun fetchAllFaces(): List<FaceImage>

    @Query("SELECT * FROM faceimage WHERE faceName = :name AND isPrimary = :isPrimary")
    suspend fun fetchImagesFor(name: String, isPrimary: Int): List<FaceImage>
